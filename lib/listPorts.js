...
import { getProcessInfo } from "./checkPort.js";
...
export async function listPorts() {
  try {
    ...
    if (process.platform === "win32") {
      output = execSync("netstat -aon | findstr LISTENING", {
        stdio: "pipe",
      }).toString();
    ...
    const ports = Array.from(portMap, ([port, pid]) => ({ port, pid })).sort(
      (a, b) => a.port - b.port
    );
    ...
    return results.length
      ? results
      : [{ status: "error", message: "No listening ports found" }];
  } catch (e) {
